...
1. 在 Supabase SQL Editor 依序執行專案根目錄的 `supabase-0001` 至 `supabase-0013` 升級檔。第二份升級會加入本期收入、上期信用卡帳單、其他收入、固定開銷規則與週期設定；第三份升級會加入固定開銷拖曳排序與每年指定月份排程；第四份升級會加入支出性質、店家分析群組與別名；第五份升級會加入待收代墊與分次收回紀錄；第六份升級會加入既有代墊的安全修改；第十份會保存每期固定開銷的不同金額；第十一份會啟用年度週期、預估年終與分紅；第十三份會加入每期生活開銷上限。升級不會刪除既有開銷，原有固定開銷會維持每月排程。
