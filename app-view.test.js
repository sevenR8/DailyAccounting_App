...
test('現金與收回款選取圓點使用一致的亮藍色', () => {
  ...
  assert.match(stylesSource, /\.reimbursement-edit-toggle input:checked[\s\S]*radial-gradient\(circle, #dce5dd 0 3px, #168df0 3px 100%\)/);
});
...
test('整張圓餅圖卡片可開啟含年度總覽的消費分析長頁', () => {
  ...
  assert.match(appSource, /每月平均收入 NT\$ \$\{formatAmount\(Math\.round\(annualDisplay\.salaryAnnualIncome \/ 12\)\)\} × 12 ＋ 分紅、年終 NT\$ \$\{formatAmount\(annualDisplay\.supplementalIncome\)\}/);
  assert.match(appSource, /平均每月可存 NT\$ \$\{formatAmount\(annualDisplay\.averageMonthlySavings\)\}/);
});

test('本期可存額會顯示收入、開銷與上月信用卡的計算式', () => {
  assert.match(appSource, /savingsBreakdown/);
  assert.match(appSource, /本月總收入/);
  assert.match(appSource, /現金開銷/);
  assert.match(appSource, /固定開銷（現金）/);
  assert.match(appSource, /本月信用卡應繳/);
  assert.match(appSource, /class="analysis-savings-breakdown"/);
  assert.doesNotMatch(appSource, /class="savings-formula"/);
});
...
test('帳務管理可設定本期生活開銷上限，並在快速記帳區顯示每日剩餘預算', () => {
  ...
  assert.doesNotMatch(appSource, /reservedFixedAmount/);
  assert.match(appSource, /上限 \$\$\{formatAmount\(dailyLivingBudget\.limit\)\}（不含固定開銷）/);
  assert.match(stylesSource, /\.daily-living-budget/);
});

test('本期收入卡片會顯示本期生活開銷上限', () => {
  assert.match(appSource, /class="income-limit-row/);
  assert.match(appSource, /本期生活開銷上限（不含固定開銷）/);
